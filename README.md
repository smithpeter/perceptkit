...
> ⚠️ **Honest status disclosure** (2026-04-19, post final-audit):
>
> This is **pre-alpha**. The framework, CLI, FSM, and Python bindings are real and tested (87 tests green, hot path 1.77 µs measured). But two of the five strategic dimensions promised by the project's v0.1 plan are **not yet delivered**:
>
> - **Accuracy**: our CI `eval --gate` currently tests synthetic data generated by the same code that defines the scenes. This is a sanity check, **not a real accuracy measurement**. Real human-labeled benchmark is not yet built.
> - **Learnability (self-learning)**: the Reflector trait is wired with `NoopReflector` and `MockReflector` only. The promised **LocalReflector (Qwen-0.5B via llama-cpp-2)** is not yet integrated.
>
> We're downgrading the version tag from `v0.1.0-alpha.0` to `v0.1.0-pre-alpha.1` until these land. See CHANGELOG.md for the audit and the path back to alpha.
